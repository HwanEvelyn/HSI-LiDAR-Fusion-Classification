...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="汇总 embed_dim 实验并生成表格与 OA 曲线图")
    parser.add_argument("summary_files", nargs="+", help="多个 results/repeats/*/summary.json 文件")
    parser.add_argument("--output-dir", type=str, default="results/paper_figures/embed_dim")
...
def infer_embed_dim(summary_path: Path) -> int:
    ...
    with repeat_config.open("r", encoding="utf-8") as f:
        config = json.load(f)
    embed_dim = config["train_args"].get("embed_dim")
    if embed_dim is None:
        raise ValueError(f"{repeat_config} 中没有 embed_dim")
...
def plot_embed_dim_vs_oa(rows: list[dict], output_path: Path) -> None:
    embed_dims = [row["embed_dim"] for row in rows]
    oa_means = [row["test_oa_mean"] * 100 for row in rows]
    oa_stds = [row["test_oa_std"] * 100 for row in rows]

    plt.figure(figsize=(6, 4))
    plt.errorbar(embed_dims, oa_means, yerr=oa_stds, marker="o", linewidth=2, capsize=4)
    plt.xlabel("embed_dim")
    plt.ylabel("OA (%)")
    plt.title("embed_dim vs OA")
...
def main() -> None:
    ...
    for summary_file in args.summary_files:
        ...
        embed_dim = infer_embed_dim(summary_path)
    ...
    rows.sort(key=lambda row: row["embed_dim"])
    markdown = build_markdown(rows)
    (output_dir / "embed_dim_table.md").write_text(markdown, encoding="utf-8")
    plot_embed_dim_vs_oa(rows, output_dir / "embed_dim_vs_OA.png")

    print(markdown)
    print(f"Saved table to {output_dir / 'embed_dim_table.md'}")
    print(f"Saved plot to {output_dir / 'embed_dim_vs_OA.png'}")
