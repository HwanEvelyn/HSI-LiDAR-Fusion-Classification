...
不过从本阶段实验总结的角度看，后文提到的“v2 主配置提升”主要是指你已经完成的这组实验事实：在 v1 主配置基础上，引入 `light_hetero + conservative fusion + aux heads` 后，测试集性能由 `OA = 85.58 ± 0.42`、`AA = 87.63 ± 0.46`、`Kappa = 84.40 ± 0.46` 提升到 `OA = 86.38 ± 0.14`、`AA = 88.52 ± 0.13`、`Kappa = 85.26 ± 0.16`。因此，文档后面在做结果分析时，重点采用的是这组实验结论，而不是仅仅按当前仓库里这份 `yaml` 文件名来理解“v2.0”。
...
3. 你已经完成了多尺度 `average / gate / residual` 的融合消融，结论是 `residual` 综合效果最好。
...
7. 当前最推荐作为 v2 正式版本的方案是：`11×17 multiscale + residual + context-token-size=11 + aux=linear`。
